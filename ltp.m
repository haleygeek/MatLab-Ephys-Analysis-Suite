...
save_summary (1,2) = cellstr('% Baseline @ 55-60 min');
...
while current_col <= max_col
    ...
    save_ltp (1, current_col) = cellstr(exp_name);
...
while col < max_col + 1;
    ...
    while row < max_row + 1;
        ...
        save_ltp (row + 1, col) = num2cell(save_data (row, col));
    ...
    save_summary (col + 1, 2) = num2cell(perc_ltp)
...
if ispc == 1
    cd (pathname)
    xlswrite (filename, save_ltp, worksheet2);
    ...
    xlwrite(filename2, save_ltp, worksheet2);
